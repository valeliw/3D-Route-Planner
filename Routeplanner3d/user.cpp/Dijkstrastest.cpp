...
//note: Node(id, type, building, floor)
{
	...
	///////////////////////map of the floors
	std::cout << " legend  -- = wall, >> = corridor, RM = room, S = stairs, L= lift, T=toilet" << std::endl;
	std::cout << "----RM01----RM02---------T01----RM03----RM04---RM05-----------------------" << std::endl;
	std::cout << "S01>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>S02-------------" << std::endl;
	std::cout << "------->>>>>>--------------->>>>>--------------------->>>>>>>>-------------" << std::endl;
	std::cout << "------->>>>>>--------------->>>>>--------------------->>>>>>>>-------------" << std::endl;
	std::cout << "------->>>>>>--------------->>>>>--------------------->>>>>>>>-------------" << std::endl;
	std::cout << "------->>>>>>--------------->>>>>--------------------->>>>>>>>-------------" << std::endl;
	std::cout << "--RM24>>>>>RM25------------->>>>>-----------------RM06>>>>>>>>-------------" << std::endl;
	std::cout << "------->>>>>>--------------->>>>>--------------------->>>>>>>>-------------" << std::endl;
	std::cout << "------->>>>>>------------>>>>>>>>>>------------------->>>>>>>>-------------" << std::endl;
	std::cout << "--RM23>>>>>>RM22--------->>>>>>>>>>------------------->>>>>>>>-------------" << std::endl;
	std::cout << "------->>>>>>---------L01>>>>>>>>>>------------------->>>>>>>>-------------" << std::endl;
	std::cout << "------->>>>>>------------>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>---------" << std::endl;
	std::cout << "---RM20>>>>>>---------L02>>>>>>>>>>---------------RM08>>>>>>>>-->>---------" << std::endl;
	std::cout << "------->>>>>>RM21-------->>>>>>>>>>L05----------------->>>>>>>>->>---------" << std::endl;
	std::cout << "------->>>>>>----L04>>--->>>>>>>>>>-------------------->>>>>>>>->>---------" << std::endl;
	std::cout << "------->>>>>>------->>----------------------------RM10>>>>>>>>-->>---------" << std::endl;
	std::cout << "--RM19>>>>>>-------->>--------------------RM11-------->>>>>>>>-->>---------" << std::endl;
	std::cout << "------->>>>>>>>>>>>>>>-------------------->>>>>>>>>>>>>>>>>>>>-->>RM09-----" << std::endl;
	std::cout << "------->>>>>>------------------------RM12>>>>RMS3----->>>>>>>>-------------" << std::endl;
	std::cout << "------->>>>>>---------------------------->>>---------->>>>>>>>-------------" << std::endl;
	std::cout << "------->>>>>>----RM17-------RM16--------->>>--------T02>>>>>>>-------------" << std::endl;
	std::cout << "--RM18>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>L03-----------" << std::endl;
	std::cout << "--------Passway(3rdflooronly)----------RM15-------RM14------RM13-----------" << std::endl;

	//asks for start postition
	std::cout << "Do you have any disabilities that would prevent the use of stairs?" << std::endl;
	bool handicap = setHandicap();
	std::cout << "input start/ stop points with prefix of building and floor eg 'TG314' to RC106" << std::endl;
	std::cout << "Input starting point id: " << std::endl;
	int startID = setStartingPoint();
	nodes.at(startID)->distanceFromStart = 0; // set beginning

	//sets the destination
	std::cout << "Input the destination point: " << std::endl;
